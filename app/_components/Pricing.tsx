...
export default function Pricing() {
  return (
    <section id="pricing" className="py-24 px-4 sm:px-6 bg-[#FAF8F4]">
          ...
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="flex items-end gap-1 mb-1">
                ...
                <span className={`text-sm pb-1 ${plan.highlight ? "text-stone-300" : "text-stone-500"}`}>
                  /month
                </span>
              </div>
              <p className={`text-sm mb-1 ${plan.highlight ? "text-stone-300" : "text-stone-500"}`}>
                or {plan.annualPrice}/year
              </p>
              <p className="text-xs font-semibold text-[#D42232] mb-4">{plan.annualNote}</p>
              <p className={`text-sm mb-6 ${plan.highlight ? "text-stone-300" : "text-stone-500"}`}>
                {plan.description}
              </p>
              ...
              {plan.trialNote && (
                <p className="text-stone-400 text-xs text-center mt-3">{plan.trialNote}</p>
              )}
            </div>
          ))}
        </div>
